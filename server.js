//=============== FROM CLI PARAMETER ===============//
var pg_user = 'postgres'//process.argv[2];
var pg_password = '@dm1n1$tr@t0r' //process.argv[3];
var ENV = 'SERVERS_SA_5'//process.argv[4];
...
var SETTINGS = {
	SERVERS_SA_5:{
		CONN_STRING:	'postgres://' + pg_user + ':' + pg_password + '@192.168.254.11:5432/geoportal',
		PORT:		8011
	},
	LAPTOP:{
...
function setAPI(){
	...
	app.get('/', basicAuth, function(req, res){
		res.sendfile(__dirname + "/app/icsu.html");
	});


	// a general proxy servicef
	...
	// generate new transaction record
	app.get('/dl/epsg/:layer_name', function(req, res){
		
		query("SELECT ST_SRID(wkb_geometry) FROM " + req.params.layer_name + " limit 1",[],function(err, result){
			res.json(result.rows?result.rows:result);
